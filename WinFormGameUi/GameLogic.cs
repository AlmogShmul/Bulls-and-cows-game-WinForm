...
namespace WinFormGameUi
{
    ...
    {
        private readonly List<Color> r_ColorList;

        private readonly List<Color> r_ListOfColorSelection;

        public List<Color> ColorList { get; set; }

        public GameLogic()
        {
            r_ListOfColorSelection = new List<Color>(4);
        ...
        public void RandomComputerSelection()
        {
            ...
            for (int index = 0; index < 4; index++)
            {
                ...
                while (!flagOfIter)
                {
                    ...
                    {
                        r_ListOfColorSelection.Add(ColorList[randomNumber]);
        ...
        public void CheckResults(RowItemForGame i_RowItemForGame, ref bool i_BoolToCheckIfWin)
        {
            ...
            {
                if(listToCheck[index].BackColor == r_ListOfColorSelection[index])
                {
                    counterOfBoolPgia++;
                }
                else if (r_ListOfColorSelection.Contains(listToCheck[index].BackColor))
                {
                    countOfPgia++;
                }
            }
        ...
        public void UpdateColorsOfComputerSelection(List<Button> i_ListButtonToUpdateColor)
        {
            for(int index = 0; index < 4; index++)
            {
                i_ListButtonToUpdateColor[index].BackColor = r_ListOfColorSelection[index];
            }
        }
    }
}